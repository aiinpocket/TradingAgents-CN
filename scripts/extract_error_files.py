...
def run_syntax_check():
    ...
    try:
        ...
        print(f"運行語法檢查時出錯 | Error running syntax check: {e}")
        return "", str(e), 1
...
def generate_report(error_files):
    ...
    """
    if not error_files:
        print("🎉 没有發現語法錯誤文件！| No syntax error files found!")
        return
    
    print(f"\n🚨 發現 {len(error_files)} 個文件存在語法錯誤 | Found {len(error_files)} files with syntax errors:\n")
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write(f"語法錯誤文件報告 | Syntax Error Files Report\n")
        f.write(f"生成時間 | Generated at: {__import__('datetime').datetime.now()}\n")
        f.write(f"錯誤文件數量 | Error files count: {len(error_files)}\n\n")
        
        f.write("詳細錯誤信息 | Detailed error information:\n")
        f.write("=" * 60 + "\n")
    ...
    print(f"\n📄 詳細報告已保存到 | Detailed report saved to: {report_file}")
...
def main():
    ...
    Main function
    """
    print("🔍 開始提取語法錯誤文件列表 | Starting to extract syntax error files list...")
    
    # 運行語法檢查
    stdout, stderr, returncode = run_syntax_check()
    
    if stderr:
        print(f"⚠️  語法檢查過程中有警告 | Warnings during syntax check: {stderr}")
