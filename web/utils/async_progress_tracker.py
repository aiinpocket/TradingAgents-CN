        ...
        try:
            ...
            def register_with_timeout():
                try:
                    ...
                    logger.debug(f"[進度整合] 追蹤器註冊成功: {self.analysis_id}")
                except Exception as e:
                    logger.debug(f"[進度整合] 追蹤器註冊失敗: {e}")
            ...
            if register_thread.is_alive():
                logger.debug(f"[進度整合] 追蹤器註冊超時，繼續執行: {self.analysis_id}")

        except ImportError:
            logger.debug("[非同步進度] 日誌整合不可用")
        except Exception as e:
            logger.debug(f"[進度整合] 追蹤器註冊異常: {e}")
