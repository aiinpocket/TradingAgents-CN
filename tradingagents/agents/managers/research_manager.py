...
簡潔地总結雙方的關键觀點，重點關註最有說服力的證據或推理。您的建議——买入、卖出或持有——必须明確且可操作。避免仅仅因為雙方都有有效觀點就默認選擇持有；要基於辩論中最强有力的論點做出承诺。
...
考慮您在類似情況下的過去錯誤。利用這些見解來完善您的決策制定，確保您在學习和改進。以對話方式呈現您的分析，就像自然說話一樣，不使用特殊格式。
