...
TradingAgents-CN 提供了完整的配置管理和成本統計系統，讓您可以：
...
| 供應商 | 模型示例 | 貨幣 | 說明 |
|--------|----------|------|------|
| **** | gpt-4o-mini, gpt-4o, gpt-4o | CNY | 模型，推薦使用 |
| **OpenAI** | gpt-3.5-turbo, gpt-4, gpt-4-turbo | USD | 國際領先模型 |
...
|--------|------|--------|
| **默認供應商** | 新分析時的默認模型供應商 | 
| **默認模型** | 默認使用的模型名稱 | gpt-4o-mini |
| **啟用成本跟蹤** | 是否記錄Token使用和成本 | 啟用 |
| **成本警告閾值** | 日成本超過此值時警告 | ¥100 |
| **首選貨幣** | 成本顯示的首選貨幣 | CNY |
...
```
總成本 = (輸入Token數 / 1000) × 輸入單價 + (輸出Token數 / 1000) × 輸出單價
```
...
**操作**:
1. 設置成本警告閾值為¥16.67（500/30天）
2. 選擇性價比高的模型（如gpt-4o-mini）
3. 定期查看使用統計，調整使用頻率
...
**操作**:
1. 配置多個模型（gpt-4o-mini, gpt-4o, gpt-3.5-turbo）
2. 對同一股票進行多次分析
3. 在使用統計中比較成本和效果
...
### 數據安全
- **API密鑰加密存儲**: 系統會安全存儲您的API密鑰
- **本地數據**: 所有配置和使用記錄都存儲在本地
- **定期備份**: 建議定期導出配置進行備份
