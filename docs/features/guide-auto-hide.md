...
為了提升用戶體驗和優化屏幕空間利用，我們實現了使用指南的智能自動隐藏功能。該功能會根據用戶的操作狀態自動調整使用指南的顯示狀態。
...
2. **開始分析**: 點擊"開始分析"按鈕後自動隐藏使用指南，節省屏幕空間
3. **分析完成**: 保持隐藏狀態，讓用戶專註於分析結果
