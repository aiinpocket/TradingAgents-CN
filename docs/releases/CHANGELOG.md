...
## [v0.4.4] - 2026-02-23 - 效能並行化 + 背景刷新 + 安全強化

### 概述

v0.4.4 對趨勢資料抓取進行並行化改造，新增背景定時刷新機制，清理未使用的 i18n 翻譯鍵，並強化安全標頭與輸入驗證。
...
#### CLS 修復
- **trending-panel min-height**: 310px → 435px，匹配實際 8 筆資料高度
- **skeleton 行數**: 5 → 8（與實際資料量一致），消除冷啟動時骨架→資料切換 CLS

#### 安全強化
- **新增 Cross-Origin-Resource-Policy**: same-origin，防止跨域資源嵌入
- **CSP 擴充**: 加入 base-uri、form-action、object-src、worker-src 限制
- **HSTS preload**: 加入 preload 指令
- **Permissions-Policy 擴充**: 新增 accelerometer/gyroscope/magnetometer/usb/bluetooth/screen-wake-lock/interest-cohort
