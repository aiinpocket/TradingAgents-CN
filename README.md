...
- **純美股專注**: 移除所有中國 AI 模型，僅保留 OpenAI 與 Anthropic Claude
- **動態供應商管理**: 支援動態添加和配置 LLM 供應商
...
- **多語言架構**: 內建 i18n 翻譯框架，預設繁體中文，支援英文切換
- **API 密鑰管理**: 所有金鑰透過 `.env` 環境變數管理，不硬編碼
...
- **v0.5.0** (2026-02-27): 報告分頁鍵盤導航修復（跳過隱藏 tab）、Markdown 渲染器強化（代碼塊邊界 + 表格列數正規化）、歷史清單載入/錯誤狀態反饋、分析事件引用終態清理（記憶體洩漏防護）、取消計時器重置修復、WCAG 2.5.8 觸控目標修復(44px)、快取時鐘回調防護、focus-visible 一致化、AI 趨勢分析跨 tab 語言切換修復、SignalProcessor 去 LLM 化（純正則提取）、CSS 觸控回饋(:active)、暗色模式對比度強化、MongoDB 非阻塞儲存、SSE 事件驅動通知、ARIA 無障礙修正、i18n 168 key 對稱
